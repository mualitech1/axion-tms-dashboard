...
export default function DragDropScheduler() {
    ...
    const updatedJobs = jobs.map(job => {
      if (job.id === draggedJob.id) {
        return { 
          ...
          status: "scheduled" as const // Fix the type error by using a const assertion
        };
      }
      return job;
    });
  ...
  return (
    <Card className="p-6 overflow-auto bg-white shadow-md border-0">
      ...
      <div className="flex mb-6 space-x-4">
        <div className="border rounded-md p-4 w-1/3 bg-muted/30">
          ...
          <div className="space-y-2">
            {jobs.filter(job => !job.assignedTo).map(job => (
              ...
              >
                <p className="font-medium text-tms-gray-800">{job.title}</p>
                <p className="text-sm text-tms-gray-600">{job.client}</p>
                <Badge variant={job.status === "pending" ? "outline" : "secondary"} className="mt-2">
                  {job.status}
                </Badge>
              </div>
            ))}
        ...
        <div className="w-2/3 border rounded-md bg-white shadow-sm">
          <Table>
